...
[14. Динамическая память](#dmemory)

[15. Работа с битами](#bit)
...
Читает из потока _stream_ и записывает в буфер _s_ строку до _'\n'_ но не более _size_ символов.

Буфер - это заранее выделенная нами память. Массив или динамический массив.
...
int main(){
    char a[10];
    fgets(a, 5, stdin);        //читаем и ЗАПИСЫВАЕМ в массив a, не более 5 символов с stdin
    printf("<%s>\n", a);       //прочитываем прочитанное
    return 0;
}
```
...
_fgets_ записывает n-1 символов, а последний - '\0'

В отличии от _scanf_ _fgets_ не нужно указывать количество -1. 

Если дать строку меньше чем размер буфера, то перед символом '\0' будет символ '\n'
...
для работы с этими функциями используется библиотека `ctype.h`

`int isalpha(int c);` - проверка маленькая или большая с

`int isdigit(int c)` - проверка на число от 0 до 9

`int isxdigit(int c);` - проверка на с от 0 до 9, a-f, A-F (проверка на hex)

`int isalnum(int c)` - проверка с - буква или цифра

`int isspace(int c)` - проверка на пробельный символ ' ' '\t' '\n' '\r' '\v' '\f'

`int isprint(int c)` - проверка на любой печатный символ (включая пробельные)

`int ispunct(int c)` - проверка на любой печатный символ, не isalnum и не isspace

`int islower(int c)` - проверка на маленькую букву a...z

`int isupper(int c)` - проверка на большую букву A...Z
...
`size_t strlen(const char *);` - возвращает количество символов в строке без подсчета '\0'

Для печати size_t используюйте %zu
...
Другие функции для работы со строками можно увидеть по команде `man 3 string`

<a name="dmemory"><h2>14. Динамическая память</h2></a>

В Си программа может неявно выделять и освобождать память, например когда объявляется переменная в функции и освобождается эта память при выходе из функции. Кроме неявной автоматической работы с памятью ее можно выделять и освобождать с помощью стандартных функций языка Си.

Это нужно, если:

- в программе используются нестандартные структуры данных, например, нужна треугольная матрица, а не прямоугольная
- во время работы программы нужно изменить размер массива
...
__Начиная с С99 можно определить длину массива на этапе выполнения__

Поэтому можно определить длину массива без динамической памяти:
...
Тоже самое можно записать через функции __malloc__ и __free__:
...
Для работы с __malloc__ и __free__ нужно подключить `stdlib.h`

Пусть надо прочитать n чисел в массив. Если n не задано и не ограничено, то непонятно массив какого размера объявлять. 

Для таких ситуаций можно динамически выделять память при помощи __realloc__. Размер массива будет расти на каждой итерации цикла.

`a = realloc(a, (i + 1)*sizeof(int))`

При этом при каждом новом вызове __realloc__ копирует данные из старой памяти в новую и освобождает старую память.
...
После того как массив перестал быть нужен, надо освободить выделенную под него память функцией __free__.
...
Тоже самое делает стандартная функция __strdup__
...
Для работы с этими функциями нужно подключить библиотеку `stdlib.h`

`void *malloc(size_t size);`

Выделяет память размером `size` байт. Возвращает указатель на выделеную память или NULL в случае ошибки.

`void *calloc(size_t nmemb, size_t size);`

Выделяет память размером `nmemb * size`. __Заполняет память 0__. Возвращает указатель на выделенную память или NULL в случае ошибки.

Разница между `malloc` и `calloc` в том что calloc гарантированно заполнит память нулями.

`void free(void *ptr);`

Освобождает память на которую указывает `ptr` (Начало динамической памяти). `free(NULL)` ничего не делает.

`void *realloc(void *ptr, size_t size);`

Выделяет память размером `size` байт. Копирует данные из памяти, на которую указывает `ptr` в новую память. Освобождает старую память. Возвращает указатель на выделенную память или NULL в случае ошибки. При `ptr = NULL` работает как `malloc`

#### Динамический двумерный массив
 
Допустим необходимо сделать копию `b` массива `a[3][4]` так, чтобы можно было обращаться b[i][j] к элементу массива

Простое копирование содержимого двумерного массива a в массив a1

![image](https://github.com/user-attachments/assets/33496539-d19e-4abd-9dc3-2398bf2e028e)
...
Чтобы можно было обращаться к массиву как `b[i][j]`: Если отдельный элемент массива `b[i][j]` имеет тип `int`, то `b[i]` имеет тип `int *`. Тоесть для массива 3 на 4:

Первые 3 элемента массива b[0] b[1] b[2] должны иметь тип int * и содержать указатель на выделенную для хранения 1 строки память

![image](https://github.com/user-attachments/assets/a078c9fc-64ed-4bb1-9d7d-ca2019156453)

Переменная b содержит указатель на массив элементов типа `int *` тоесть ее тип `int **`

![image](https://github.com/user-attachments/assets/ff72006a-9ea3-474f-b48b-0d065525f0ff)
...
<a name="bit"><h2>15. Работа с битами</h2></a>

#### Перевод из одной системы счисления в другую

При переводе из одной системы счисления в другую удобно переводить сначало в десятичную, а потом в конечную систему счисления. Тоесть если нужно перевести число 011 из восьмеричной системы в двоичную можно сначало перевести его в десятичную систему, а потом в двоичную.

Для перевода из десятичной системы нужно __умножать на исходное основание__. Для перевода в десятичную систему нужно __делить на новое основание__ и записывать остатки от деления как новое число.

Например, переведем число `A3` из _hex_ в _bin_ формат:

1) Переводим в _dec_ формат:

`A3 = A * 16^1 + 3 * 16^0 = 10 * 16 + 3 * 1 = 160 + 3 = 163` 
    
2) Переводим из _dec_ в _bin_
...
Также можно переводить числа из одной системы счисления в другую, зная что _hex_ занимает 4 бита (0-15 или 0x0-0xF), _oct_ занимает 3 бита (от 0-7 или 00-07).

#### Запись и печать чисел:

Константы в си имеют тип `int` (dec) или unsigned int и записываются:

- 12 в десятичной системе. Формат `%d`
- 012 - в восьмеричной системе (Начинается с 0). Формат `%o`
- 0x12 - в шестнадцатеричной системе (начинается с 0x). Формат `%x`
- Бинарный тип в стандарте не поддерживается (в GNU используется префикc 0b)

#### Побитовые операции

![image](https://github.com/user-attachments/assets/e947ff95-e0a8-4a3e-83e7-7697a68607dd)

##### Побитовое ИЛИ `|` используют если нужно установить 1 в нужных битах, не трогая значения остальных бит

![image](https://github.com/user-attachments/assets/3793eb2f-ef8d-4a4d-b54e-5b0af0ef0d8a)

##### Побитовое И `&` используют если нужно установить 0 в определенных битах, не трогая значения остальных бит. Или обнулить значение ненужных бит числа 0xA, оставив только те биты, где в числе 7 были единицы (взять число 0xA по маске 7)

![image](https://github.com/user-attachments/assets/86fd8d4e-267a-4297-92ae-077ad1286e36)

##### Побитовое отрицание '~' меняет 0 на 1 и 1 на 0. Используется для создания масок. Например, если нужно обнулить первые три бита числа можно использовать маску `~7`

![image](https://github.com/user-attachments/assets/d73e6fb2-8351-4c5a-8c59-e8f5abcfc29a)

##### Побитовое исключающее или `^`. Выдаст 1 только если один из битов 1. В остальных случаях выдаст 0.

![image](https://github.com/user-attachments/assets/0d06bc50-87a7-449d-a7e4-304c63b47b89)

##### Побитовый сдвиг влево `<<`

Этот сдвиг иногда называют побитовым умножением, поскольку он эквивалентен умножению на 2^n. n - целочисленное

__При сдвиге влево `<<` в конце всегда дописывается 0.__

![image](https://github.com/user-attachments/assets/78c51d34-b838-4268-913b-b69d098215ef)

![image](https://github.com/user-attachments/assets/a53b4ab4-fef9-46da-9296-6df9d598ea02)

##### Побитовый сдвиг вправо `>>`

Этот сдвиг иногда называют побитовым делением, поскольку он эквивалентен делению на 2^n. n - целочисленное

__Беззнаковое число всегда дополняется 0, знаковое число дополняется знаковым битом__

![image](https://github.com/user-attachments/assets/ba6b2305-d6bc-45c1-b3d1-f315fe65896b)

![image](https://github.com/user-attachments/assets/60af43c2-6155-4e8a-9370-d75d294561d8)
