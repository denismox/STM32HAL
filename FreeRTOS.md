...
`USE_RECURSIVE_MUTEXES` - позволяет задаче взять один мьютекс несколько раз подряд если . Если задача А и Б обе блокируют один и тот же мьютекс, и А вызывает Б то без включенного параметра произойдет deadlock.

#### USE_COUNTING_SEMAPHORES 

`USE_COUNTING_SEMAPHORES` - включает поддержку счетных семафоров. Рекомендуется включать всегда тк нагрузка минимальна.
